...
StoreInst::StoreInst(Value *val, Value *ptr, BasicBlock *bb)
    : BaseInst<StoreInst>(bb->get_module()->get_void_type(), store, bb) {//, bb
...
StoreInst *StoreInst::create_store(Value *val, Value *ptr, BasicBlock *bb) {//, BasicBlock *bb
    return create(val, ptr, bb); //, bb
}
