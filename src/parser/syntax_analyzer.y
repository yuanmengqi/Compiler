...
%token <node> LPAREN RPAREN LBRACKET RBRACKET LBRACE RBRACE COMMA SEMI LT GT ASSIGN
%token <node> COMMENT LE GE EQ NE ELSE IF INT RETURN VOID WHILE FLOAT ID
%token <node> NUMBERFLOAT
%token <node> INTEGER
...
var-declaration : type-specifier ID SEMI {$$ = node("var-declaration", 3, $1, $2, $3);}
                | type-specifier ID LBRACKET INTEGER RBRACKET SEMI {$$ = node("var-declaration", 6, $1, $2, $3, $4, $5, $6);}
                ;
...
integer : INTEGER {$$ = node("integer", 1, $1);}
        ;
float : NUMBERFLOAT {$$ = node("float", 1, $1);}
        ;
