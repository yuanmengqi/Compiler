...
void Mem2Reg::generate_phi() {
    ...
    for (auto &global_name : global_names) {
        if (global_name.second.size() > 1) {
            mem2reg_global_names_multi.insert(global_name);
        }
    }
    //将得到的map赋值给类的成员变量
    ...
    for (auto &v : mem2reg_global_names_multi) {
        //F<-{}
        ...
        while(!W.empty()){
            ...
            for(auto &y : next){ //auto &y : dominators_->get_dominance_frontier(x)
                ...
                if(F.find(y) == F.end()){
                    ...
                    auto phi = PhiInst::create_phi(v.first->get_type()->get_pointer_element_type(), y); //v.first->get_type()!!!!!!
...
void Mem2Reg::rename(BasicBlock *bb) {
    ...
    for(auto &v : mem2reg_global_names_multi){
        ...
        mem2reg_global_names_stack.insert({v.first, stack});
    }
...
void Mem2Reg::Rename(BasicBlock *bb) {
    ...
    for(auto &instr : bb->get_instructions()) {
        ...
        if (instr.is_phi()) {
            ...
            if(auto *global = dynamic_cast<GlobalVariable *>(k))
                keep = true;
            if(keep == false)
                j->replace_all_use_with(mem2reg_global_names_stack[k].top());
        }
    ...
    for(auto &succ : next){//bb->get_succ_basic_blocks()
        ...
        for(auto &instr : succ->get_instructions()){
            if(instr.is_phi()){
                ...
                phi_wait = static_cast<PhiInst *>(&instr);
                auto phi_inst = phi_record.find(phi_wait);
                std::cout << phi_inst->first << " " << phi_inst->second << std::endl;
                if(mem2reg_global_names_stack.find(phi_inst->second) == mem2reg_global_names_stack.end()
                  || mem2reg_global_names_stack[phi_inst->second].empty()){
                    //succ->erase_instr(&instr);
                    continue;
                }
                std::cout << mem2reg_global_names_stack[phi_inst->second].top() << std::endl;
                auto val = static_cast<Value *>(mem2reg_global_names_stack[phi_inst->second].top());
