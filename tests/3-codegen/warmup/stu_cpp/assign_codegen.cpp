...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("ld.d",
                         {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    //codegen->append_inst("ld.d $t1, $t0, 0");
    // TODO: 计算 %op1 的值, 并将其写入 %op1 对应的内存空间中
    codegen->append_inst("addi.d",
                         {"$t2", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("st.d $t0, $t2, 0");
    ...
    codegen->append_inst("addi.d $t2, $t0, 4");
    codegen->append_inst("addi.d",
                         {"$t3", "$fp", std::to_string(offset_map["%op2"])});
    codegen->append_inst("st.d $t2, $t3, 0");
    ...
    codegen->append_inst("addi.d",
                         {"$t2", "$fp", std::to_string(offset_map["%op3"])});
    codegen->append_inst("st.w $t1, $t2, 0");
    ...
    //codegen->append_inst("ld.w $t1, $t0, 0");
    codegen->append_inst("addi.w $t2, $zero, 3");
    codegen->append_inst("mul.w $t3, $t0, $t2");
    codegen->append_inst("addi.d",
                         {"$t4", "$fp", std::to_string(offset_map["%op4"])});
    codegen->append_inst("st.w $t3, $t4, 0");
    ...
    //codegen->append_inst("ld.d $t3, $t2, 0");
    codegen->append_inst("st.w $t0, $t2, 0");

    /* %op5 = load i32, i32* %op2 */
    // 将 %op2 指向的空间中存储的值写入 %op5 对应的内存空间中
    offset_map["%op5"] = -96; // TODO: 请填空
    ...
    //codegen->append_inst("ld.d $t1, $t0, 0");
    codegen->append_inst("addi.d",
                         {"$t2", "$fp", std::to_string(offset_map["%op5"])});
    codegen->append_inst("ld.w $t3, $t0, 0");
    codegen->append_inst("st.w $t3, $t2, 0");
    ...
    codegen->append_inst("ld.w $a0, $t0, 0");
