...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("b .store1");
    codegen->append_inst(".store_0", ASMInstruction::Label);
    codegen->append_inst("addi.w $t0, $zero, 0");
    codegen->append_inst(".store1", ASMInstruction::Label);
    codegen->append_inst("st.w $t0, $fp, -24");
    ...
    codegen->append_inst("ld.w $t0, $fp, -24");
    codegen->append_inst("xori $t1, $t0, 1");//t0是1则t1为0
    codegen->append_inst("slti $t2, $t1, 1");//t0是1则t2为1
    codegen->append_inst("st.w $t2, $fp, -28");
    ...
    codegen->append_inst("bnez $t2, .main_label3");//等于0则跳转
    codegen->append_inst("b .main_label4");//不等于0则跳转
